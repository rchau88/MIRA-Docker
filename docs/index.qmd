...
subtitle: "<a href='https://github.com/nbx0/MIRA'>https://github.com/nbx0/MIRA</a>"
...
digraph nicegraph {
  ...
  subgraph cluster0 {
    ...
    subgraph cluster1 {
      ...
      subgraph cluster2 {
        ...
        fontsize = 120;
        color = none;
        MIRA [label = 'MIRA\\n(GUI)', width = 30, fontsize = 120, tooltip = 'a GUI web interface that allows users to interactively create a metadata and config file for running Influenza Genome and SARS-CoV-2 Spike-Gene assembly and curation'];
      }
    }
    ...
    subgraph cluster3 {
      ...
      subgraph cluster4 {
        ...
        SPYNE [label = 'SPYNE\\n(Snakemake Manager)', tooltip = 'a Snakemake workflow manager designed for running Influenza Genome and SARS-CoV-2 Spike-Gene assembly',  width = 30, fontsize = 120];
      ...
      subgraph cluster5 {
        ...
        IRMA [label = 'IRMA\\n(Genome and Spike-Gene Assembly)', tooltip = 'designed for the robust assembly, variant calling, and phasing of highly variable RNA viruses. IRMA is deployed with modules for influenza, ebolavirus and coronavirus.', width = 30, fontsize = 120];
      }
      ...
      subgraph cluster6 {
        ...
        DAIS [label = 'DAIS-Ribosome\\n(Curation)', tooltip = 'compartmentalizes the translation engine developed for the CDC Influenza Division protein analytics database. The tool has been extended for use with Betacoronavirus.', width = 20, fontsize = 120];
      }
  ...
  SOCKET -> MIRA [arrowhead = none, arrowtail = none, style = dashed, color='blue', alpha = 0.5]
  SOCKET -> IRMA [arrowhead = none, arrowtail = none, style = dashed, color='blue', alpha = 0.5]
  SOCKET -> DAIS [arrowhead = none, arrowtail = none, style = dashed, color='blue', alpha = 0.5]
  MIRA -> DATA [color = 'black']
  IRMA -> DATA [color = 'black']
  DAIS -> DATA [color = 'black']
  SPYNE -> DATA [color = 'black']
  CURATION -> DATA [color = 'black']
...
Below is an example of a Dockerfile that is used to built the MIRA image.
...
To build a docker image for <b>MIRA</b>, `cd` to where <b>Dockerfile</b> is stored and run the following command line.
...
To check if \`MIRA\` container is built successfully

``` bash
docker container ps
```

To access \`MIRA\` dashboard on the host network

Type in **http://localhost:8080** in your preferred web browse and check if the dashboard is indeed hosted there

## 2. <a href="https://docs.docker.com/compose/">Docker-Compose</a>

***Docker Compose*** is a tool for running multi-container applications on Docker defined using the Compose file format. Default is <b>docker-compose.yaml</b>.
...
To start all containers at once, `cd` to where <b>docker-compose.yaml</b> is stored and run the following command line.
